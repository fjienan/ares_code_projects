...
int main() {
    ...
    while(1){
        // int shoot_rpm;
        // std::cout << "Shoot RPM: (int)";
        // std::cin >> shoot_rpm;
        // std::cout << std::endl << "Shoot RPM: " << shoot_rpm << std::endl; 
		// // --- 示例：上位机发送执行命令 (可以按需保留或移除) ---
		// std::cout << "Sending Exec command (FuncID=0x3)..." << std::endl;
		// if (!proto.send_exec(0x3, shoot_rpm, shoot_rpm, shoot_rpm, 0x01)) {
